...
LEGACY_ROOT="${1:-legacy}"

echo "[SCAN] legacy root: $LEGACY_ROOT"
[ -d "$LEGACY_ROOT" ] || { echo "Legacy root not found: $LEGACY_ROOT"; exit 1; }

mkdir -p scripts src/lib/formulas src/lib/registry src/components/CalculatorRenderer

# --- write (or overwrite) the generator, typed, no template literals, ASCII-safe filenames ---
      ...
      while(stack.length){
        ...
        for(const e of ents){
          ...
          if(/page\.(tsx|ts|js|jsx|mdx?)$/.test(p)){
            ...
            const rel = path.relative(root, path.dirname(p)); // e.g. "fisco/imu"
            if(!rel || rel.startsWith('(') || rel.startsWith('[')) continue;
            out.push({ locale, slug: rel.replace(/\\/g,'/'), file:p });
          }
        }
      }
    }
...
# --- run it ---
GC_LEGACY_DIR="$LEGACY_ROOT" node scripts/generate_calculators.mjs

echo "[OK] Generation done. Now build:"
echo "     npm run build"
